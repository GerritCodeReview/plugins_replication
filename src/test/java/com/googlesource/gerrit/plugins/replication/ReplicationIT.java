...
public class ReplicationIT extends ReplicationDaemon {
  ...
  public void shouldReplicateNewBranchWithTemplate() throws Exception {
    ...
    Project.NameKey targetProject = createTestProject(project + "replica");
    String newBranch = "refs/heads/mybranch";
    String expectedBranch = String.format("refs/%s/heads/mybranch", project);
    String master = "refs/heads/master";
    BranchInput input = new BranchInput();
    input.revision = master;
    gApi.projects().name(project.get()).branch(newBranch).create(input);
  ...
  public void shouldReplicateNewBranchToURLWithMultipileConfiguration() throws Exception {
    ...
    try (Repository repo = repoManager.openRepository(targetProject);
      ...
      Ref masterRef = getRef(sourceRepo, master);
      for (String expectedBranch : List.of(expectedFoo1Branch, expectedFoo2Branch)) {
        Ref targetBranchRef = getRef(repo, expectedBranch);
        assertThat(targetBranchRef).isNotNull();
        assertThat(targetBranchRef.getObjectId()).isEqualTo(masterRef.getObjectId());
      }
  ...
  public void pushAllWaitCancelNotRunningTask() throws Exception {
    ...
    assertThat(getProjectTasks().size()).isEqualTo(1);
