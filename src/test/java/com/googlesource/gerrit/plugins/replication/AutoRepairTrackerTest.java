...
public class AutoRepairTrackerTest {
  ...
  public void setUp() throws Exception {
    ...
    when(replicationConfig.getAutoRepairIntervalDays()).thenReturn(3);
  ...
  public void allowsRepairUpToMaxAttemptsPerDestination() {
    when(replicationConfig.getAutoRepairIntervalDays()).thenReturn(0);
    for (int i = 0; i < 5; i++) {
      assertThat(tracker.tryBeginRepair(PROJECT, uri1)).isTrue();
    }
    assertThat(tracker.tryBeginRepair(PROJECT, uri1)).isFalse();
  }
  ...
  public void tracksAttemptsSeparatelyPerDestination() {
    ...
    assertThat(tracker.tryBeginRepair(PROJECT, uri1)).isTrue();
    assertThat(tracker.tryBeginRepair(PROJECT, uri1)).isFalse();
    assertThat(tracker.tryBeginRepair(PROJECT, uri2)).isTrue();
  }
  ...
  public void rejectsNonCopyableDestination() throws Exception {
    ...
    assertThat(tracker.tryBeginRepair(PROJECT, httpUri)).isFalse();
  }
}
