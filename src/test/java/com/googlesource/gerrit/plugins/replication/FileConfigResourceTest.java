...
public class FileConfigResourceTest {
  ...
  public void updateEmptyFile() throws Exception {
    ...
    Config restConfig = configResource.update(configUpdate);
    Config updatedConfig = newFileConfigResource().getConfig();

    assertEmptyConfig(restConfig);
  ...
  static void assertEmptyConfig(Config config) {
  ...
  private void assertConfigUpdate(Config config) {
    assertThat(config.getInt(UPDATABLE_KEY, null, VALUE_KEY, -1)).isEqualTo(1);
  }
