...
public class ReplicationEventsIT extends ReplicationDaemon {
  ...
  public void shouldEmitProjectDeletionEventsForMultipleRemotes() throws Exception {
    ...
    if (scheduled.size() != 2) {
      assertWithMessage(
              String.format(
                  ...
                  testDispatcher.projectEventsString(projectName)))
          .fail();
    }
    ...
    final List<ProjectEvent> success =
        testDispatcher.getMatching(projectName, ProjectDeletionReplicationSucceededEvent.class);

    if (success.size() != 2) {
      assertWithMessage(
              String.format(
                  "Expected two success events. Events list is:\n%s",
                  testDispatcher.projectEventsString(projectName)))
          .fail();
    }

    final List<ProjectEvent> done =
        testDispatcher.getMatching(projectName, ProjectDeletionReplicationDoneEvent.class);
