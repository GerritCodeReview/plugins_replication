...
public class ReplicationEventsIT extends ReplicationDaemon {
  ...
  public void shouldEmitProjectDeletionEventsForOneRemote() throws Exception {
    ...
    assertThat(
            testDispatcher.getMatching(
                projectNameDeleted, ProjectDeletionReplicationScheduledEvent.class))
        .hasSize(1);
    ...
    assertThat(
            testDispatcher.getMatching(
                projectNameDeleted, ProjectDeletionReplicationRunningEvent.class))
        .hasSize(1);
    assertThat(
            testDispatcher.getMatching(
                projectNameDeleted, ProjectDeletionReplicationSucceededEvent.class))
        .hasSize(1);
    assertThat(
            testDispatcher.getMatching(
                projectNameDeleted, ProjectDeletionReplicationDoneEvent.class))
        .hasSize(1);
  }
  ...
  public void shouldEmitProjectDeletionEventsForMultipleRemotes() throws Exception {
    ...
    waitUntil(
        () -> !nonEmptyProjectExists(replicaProject1) && !nonEmptyProjectExists(replicaProject2));
