...
public class ReplicationStorageIT extends ReplicationStorageDaemon {
  ...
  public void shouldNotLeakTasksForSkippedRefPermsReplication() throws Exception {
    ...
    // Create a new project which will get replicated. But since we replicate
    // as anonymous users only the master branch will replicate not
    // refs/meta/config.
    Project.NameKey sourceProject = createTestProject("user_perms_no_leaks_src_project");
    Project.NameKey targetProject = Project.nameKey(sourceProject + "replica.git");

    WaitUtil.waitUntil(() -> nonEmptyProjectExists(targetProject), TEST_NEW_PROJECT_TIMEOUT);
    ProjectInfo replicaProject = gApi.projects().name(targetProject.get()).get();
    assertThat(replicaProject).isNotNull();
    ...
    try (Repository targetRepo = repoManager.openRepository(targetProject);
      ...
      WaitUtil.waitUntil(
          () -> checkedGetRef(targetRepo, "refs/heads/master") != null, TEST_NEW_PROJECT_TIMEOUT);
    ...
    // TODO Improve this test by updating only refs/meta/config on sourceProject
    // and check that it continues to not replicate to targetProject.
