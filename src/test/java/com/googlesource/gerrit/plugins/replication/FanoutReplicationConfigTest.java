...
public class FanoutReplicationConfigTest extends AbstractConfigTest {
  ...
  @Test
  public void shouldSkipRemoteConfigFromReplicationFile() throws Exception {
  ...
  @Test
  public void shouldLoadDestinationsOnlyFromConfigFiles() throws Exception {
    ...
    config =
        new FileBasedConfig(
            sitePaths.etc_dir.resolve("replication.remotes.d/" + remoteName2 + ".yaml").toFile(),
            FS.DETECTED);
    config.setString("remote", null, "url", remoteUrl2);
    config.save();
  ...
  public void shouldThrowExceptionWhenInvalidUrl() throws Exception {
    FileBasedConfig config = newRemoteConfig(remoteName1);
  ...
  protected FileBasedConfig newRemoteConfig(String configFileName) {
    return new FileBasedConfig(
        sitePaths.etc_dir.resolve("replication.remotes.d/" + configFileName + ".config").toFile(),
        FS.DETECTED);
  }
}
