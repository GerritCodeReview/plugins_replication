...
public class ChainedSchedulerTest {
  ...
  @Test
  public void emptyCompletesRightAway() {
  ...
  @Test
  public void oneItemCompletesFine() throws Exception {
    ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1);
    TestRunner runner = new TestRunner();
    List<String> items = new ArrayList<>();
    items.add(FIRST);
  ...
  @Test
  public void manyItemsAllCompleteFine() throws Exception {
    ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1);
    TestRunner runner = new TestRunner();
  ...
  public void makesProgressEvenWhenSaturatedByOtherTasks() throws Exception {
    ...
    for (int i = 1; i <= items.size(); i++) {
      for (int b = 0; b < blockSize; b++) {
        ...
        // one batch is executed, and the item the executes, there
        // is still at least a full batch waiting.
        batches.add(executeWaitingRunnableBatch(batchSize, executor));
