...
/**
 * The intention of this series of tests, is to test the serialization of the Http request sent to
 * the plugin which results in the validation and creation of the StartReplicationRequest. The
 * processing that occurs after this class is generated, is tested in the companion  class,
 * StartReplicationProcessingTest.
 */

@RunWith(MockitoJUnitRunner.class)
public class ParseStartReplicationRequestTest {

  @Mock private StartReplicationProcessing startReplicationProcessingMock;
  @Mock private HttpServletRequest requestMock;
  @Mock private HttpServletResponse responseMock;

  private EventRestApiServlet servlet;
  private ArgumentCaptor<StartReplicationRequest> acStartReplicationRequest =
      ArgumentCaptor.forClass(StartReplicationRequest.class);
  private StartReplicationRequest startReplicationRequest=null;

  @Before
  public void setUp() throws Exception {
    servlet = new EventRestApiServlet(startReplicationProcessingMock, startReplicationRequest);
  }
  ...
  public void testStartAll() throws IOException {
    ...
    String receivedCommand = acStartReplicationRequest.getValue().getCommand();
    assertEquals(cmd, receivedCommand);

    String receivedProject = acStartReplicationRequest.getValue().getProject();
    assertEquals(project, receivedProject);

    String receivedUrl = acStartReplicationRequest.getValue().getUrl();
  ...
  public void testStartProjectWait() throws IOException {
    ...
    String receivedUrl = acStartReplicationRequest.getValue().getUrl();
    assertEquals(url, receivedUrl);
    assertTrue(acStartReplicationRequest.getValue().isWait());
  }
  ...
  public void testStartProjectDoNotWait() throws IOException {
    ...
    String receivedUrl = acStartReplicationRequest.getValue().getUrl();
    assertEquals(url, receivedUrl);
    assertFalse(acStartReplicationRequest.getValue().isWait());
  }
  ...
  public void testStartAllNow() throws IOException {
    ...
    String receivedUrl = acStartReplicationRequest.getValue().getUrl();
    assertEquals(url, receivedUrl);
    assertTrue(acStartReplicationRequest.getValue().isNow());
  }
  ...
  public void testStartAllNotNecessarilyNow() throws IOException {
    ...
    String receivedUrl = acStartReplicationRequest.getValue().getUrl();
    assertEquals(url, receivedUrl);
    assertFalse(acStartReplicationRequest.getValue().isNow());
  }
  ...
  private void doPostHelper(String Message) throws IOException {
    when(requestMock.getReader()).thenReturn(new BufferedReader(new StringReader(Message)));
    servlet.doPost(requestMock, responseMock);
    verify(startReplicationProcessingMock)
        .execute(acStartReplicationRequest.capture(), any(HttpServletResponse.class));
  }
}
