...
public class ReplicationDistributorIT extends ReplicationStorageDaemon {
  ...
  private static final int TEST_DISTRIBUTION_INTERVAL_TIMEOUT = TEST_DISTRIBUTION_INTERVAL + 1;
  ...
  public void distributorAddingTaskFromStorage() throws Exception {
    ...
    tasksStorage.create(ref); // Puts the ref obtained into storage.
    WaitUtil.waitUntil(
        () -> getProjectTasks().size() != 0,
        Duration.ofSeconds(TEST_DISTRIBUTION_INTERVAL_TIMEOUT));
