...
public class ReplicationDistributorIT extends ReplicationStorageDaemon {
  ...
  public void distributorAddingTaskFromStorage() throws Exception {
    ...
    createBranchWithoutReplicating(remote, replica, project.get(), master, newBranch);
    ...
    assertThat(waitForProjectTasks(0, TEST_PUSH_TIMEOUT)).isTrue(); // replication work completed
  ...
  private void createBranchWithoutReplicating(
      String remote, String replica, String project, String fromRef, String refToCreate)
      throws Exception {
    setReplicationDestination(remote, replica, ALL_PROJECTS, Integer.MAX_VALUE);
    reloadConfig();

    createBranchWithRevision(BranchNameKey.create(project, refToCreate), fromRef);
    for (ReplicationTasksStorage.ReplicateRefUpdate refUpdate : listWaiting()) {
  ...
  private boolean waitForProjectTasks(int count, Duration duration) {
    try {
