...
public class ReplicationQueue implements
  ...
  public void onNewProjectCreated(NewProjectCreatedListener.Event event) {
    ...
    for (Map.Entry<String, URIish> entry : getURIs(
        new Project.NameKey(projectName),
        FilterType.PROJECT_CREATION).entries()) {
  ...
  public void onProjectDeleted(ProjectDeletedListener.Event event) {
    ...
    for (Map.Entry<String, URIish> entry : getURIs(
        new Project.NameKey(projectName),
        FilterType.PROJECT_DELETION).entries()) {
  ...
  private void updateHead(Project.NameKey project, String newHeadName) {
    for (Map.Entry<String, URIish> entry : getURIs(project,
        FilterType.ALL).entries()) {
      updateHead(entry.getKey(), entry.getValue(), project.get(), newHeadName);
    }
  }

  private Multimap<String, URIish> getURIs(Project.NameKey projectName,
