...
public enum UrlDistributionStrategy {
   ...
   * when multiple replica hosts share a single backend (likely via NFS): pushing to all URLs
   * would cause redundant writes to the same underlying storage, while round-robin distributes
   * load evenly and ensures each push is executed exactly once.
   *
  ...
  UrlDistributionStrategy(String value) {
    this.value = value;
  }
