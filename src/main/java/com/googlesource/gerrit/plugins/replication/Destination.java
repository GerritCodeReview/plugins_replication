...
public class Destination {
  ...
  void schedule(
    ...
    synchronized (stateLock) {
      ...
      if (task == null) {
        ...
        if (config.requiresRemoteUrlTemplate()) {
  ...
  URIish getURI(URIish template, Project.NameKey project) throws URISyntaxException {
    return getURI(
        template, project, config.getRemoteNameStyle(), config.requiresRemoteUrlTemplate());
  }
  ...
  static URIish getURI(
      ...
      boolean requiresRemoteUrlTemplate)
    ...
    String replacedPath =
        replaceName(remoteNameStyle, template.getPath(), name, requiresRemoteUrlTemplate);
  ...
  boolean requiresRemoteUrlTemplate() {
    return config.requiresRemoteUrlTemplate();
  }
