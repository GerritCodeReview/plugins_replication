...
public class Destination {
  ...
  void schedule(
      ...
      boolean now) {
    schedule(project, refs, uri, urlMatch, state, now, false);
  }
    ...
    PushOne task =
        stateLock.withWriteLock(
            ...
            () -> {
              ...
              if (t == null) {
                t = opFactory.create(project, uri, urlMatch);
  ...
  void reschedule(PushOne pushOp, RetryReason reason) {
            ...
            switch (reason) {
              ...
              default:
                ...
                if (pushOp.setToRetry()) {
                  URIish nextUri =
                      RetryReason.TRANSPORT_ERROR.equals(reason)
                          ? urlDistributor.failover(
                              getURIs(pushOp.getProjectNameKey(), pushOp.getUrlMatch()),
                              pushOp.getURI())
                          : pushOp.getURI();
                  if (!nextUri.equals(pushOp.getURI())) {
                    status.failoverUri = nextUri;
    ...
    if (status.failoverUri != null) {
      failoverTo(pushOp, status.failoverUri);
    }
    if (status.isFailed) {
  ...
  private void failoverTo(PushOne pushOp, URIish newUri) {
    PushOne replacement =
        opFactory.create(pushOp.getProjectNameKey(), newUri, pushOp.getUrlMatch());
    replacement.addRefBatches(pushOp.getRefs());
    replacement.addStates(pushOp.getStates());
    replacement.setRetryCount(pushOp.getRetryCount());
    pushOp.removeStates();
