...
public class Destination {
  ...
  private boolean shouldReplicate(ProjectState state, CurrentUser user)
    ...
    try {
      ...
      repLog.atFine().log("Project %s is not visible to current user", name);
      return false;
    }
  }
              ...
              new Callable<Boolean>() {
                ...
                public Boolean call() throws NoSuchProjectException, PermissionBackendException {
                  ...
                  if (projectState == null) {
                    repLog.atFine().log("Project %s not found in cache", project);
                  ...
                  try {
                    ...
                    repLog.atFine().log(
                        "Ref %s on project %s is not visible to calling user", ref, project);
                    return false;
                  }
                  return true;
                }
  ...
  boolean wouldPushProject(Project.NameKey project) {
    if (!shouldReplicate(project)) {
      repLog.atFine().log("NOT replicating project %s", project.get());
      return false;
    }
    ...
    if (!matches) {
      repLog.atFine().log("NOT replicating project %s; does not match filter", project.get());
    }
    return matches;
  }
