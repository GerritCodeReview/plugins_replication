...
public class Destination {
  ...
  private PushOne getPendingPush(Project.NameKey project, URIish uri) {
    return getPendingPushByKey(PushOne.formatKey(project, uri));
  }

  @Nullable
  private PushOne getPendingPushByKey(String key) {
  ...
  void reschedule(PushOne pushOp, RetryReason reason) {
    synchronized (stateLock) {
      PushOne pendingPushOp = getPendingPushByKey(pushOp.getKey());
