...
class Destination {
   ...
   * URI than one marked as "retyring," it adds to the one
   * pending the refs list of the parameter instance.
   * <p>
   * In case the PushOp instance to be scheduled has same
   * URI than one pending, but not marked "retyring," it
  ...
  boolean requestRunway (PushOne op) {
    synchronized (stateLock) {
      if (op.wasCanceled()) {
        // Return true here so op doesn't reschedule itself.
        return true;
      }
      if (inFlight.get(op.getURI()) != null) {
        return false;
      }
