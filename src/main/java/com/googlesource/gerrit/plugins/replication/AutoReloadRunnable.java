...
public class AutoReloadRunnable<T extends RemoteConfiguration, R extends ReplicationQueueInterface> implements Runnable {
  ...
  public AutoReloadRunnable(
      ReplicationConfigValidator<T> configValidator,
  ...
  synchronized void reload() {
    ...
    try {
      ...
      final List<T> newValidEndpoints = configValidator.validateConfig(newConfig);
      loadedConfig = newConfig;
      loadedConfigVersion = newConfig.getVersion();
      lastFailedConfigVersion = "";
      eventBus.post(newValidEndpoints);
    } catch (Exception e) {
