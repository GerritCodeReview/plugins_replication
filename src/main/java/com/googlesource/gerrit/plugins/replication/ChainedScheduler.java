...
public class ChainedScheduler<T> {
  ...
  protected boolean scheduleNext() {
     ...
     * Concurrecny note:
     *
     * <p>Since there is only one chain of tasks and each task submits the next task to the
     * executor, the calls from here to the iterator.next() call will never be executed concurrently
     * by more than one thread.
     *
     * <p>Data synchronization note:
     *
     * <p>This section in the [1] javadoc: "Actions in a thread prior to the submission of a
     * Runnable to an Executor happen-before its execution begins..." guarantees that since the
     * iterator.next() happens before the schedule(), and the new tasks call to hasNext() happen
     * after the submission, that the hasNext() call will see the results of the previous next()
     * call even though it may have happened on a different thread.
     *
     * <p>[1]
