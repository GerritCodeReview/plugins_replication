...
public class SecureCredentialsFactory implements CredentialsFactory {
  ...
  private String getConfigValue(String section, String subsection, String name) {
    ...
    String val = getConfigValueFromRemoteDestination(subsection, name);

    if (val == null) {
      throw new RuntimeException(
          new ConfigInvalidException(
              "No configuration available for remote: " + subsection + " name: " + name));
    }

    return val;
  }

  private @Nullable String getConfigValueFromRemoteDestination(
      String remoteName, String propertyName) {
    try {
