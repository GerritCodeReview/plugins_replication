...
class SecureCredentialsFactory implements CredentialsFactory {
  ...
  public boolean validate(String remoteName) {
    try {
      secureStore.get("remote", remoteName, "username");
      secureStore.get("remote", remoteName, "password");
      return true;
    } catch (Throwable t) {
      log.atSevere().withCause(t).log("Credentials for replication remote %s are invalid", remoteName);
      return false;
    }
  }
}
