...
public class GerritSshApi {
  ...
  private final Set<String> withoutDeleteProjectPlugin = new HashSet<>();
  ...
  protected boolean deleteProject(String remoteName, URIish uri, String projectName) {
    if (!withoutDeleteProjectPlugin.contains(remoteName)) {
      OutputStream errStream = sshHelper.newErrorBufferStream();
      String cmd = "deleteproject delete --yes-really-delete --force " + projectName;
      int exitCode = -1;
      try {
        exitCode = execute(remoteName, uri, cmd, errStream);
      } catch (IOException e) {
      ...
      if (exitCode == 1) {
        log.info(
            "DeleteProject plugin is not installed on {} will not try to forward this opperation to that host",
            remoteName);
        withoutDeleteProjectPlugin.add(remoteName);
        return true;
      }
    }
    return true;
  ...
  private int execute(String remoteName, URIish uri, String cmd, OutputStream errStream)
      throws IOException {
    try {
      URIish sshUri = toSshUri(uri);
      return sshHelper.executeRemoteSsh(remoteName, sshUri, cmd, errStream);
