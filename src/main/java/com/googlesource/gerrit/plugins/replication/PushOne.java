...
class PushOne implements ProjectRunnable, CanceledWhileRunning {
  ...
  private PushResult pushVia(Transport tn)
    ...
    repLog.atSevere().log("Push to %s references: %s", uri, lazy(() -> refUpdatesForLogging(todo)));
    ...
    if (config.isMirror()) {
      for (Ref ref : remote.values()) {
        if (Constants.HEAD.equals(ref.getName())
            || RefNames.REFS_USERS_SELF.equals(ref.getName())) {
          repLog.atFine().log("Skipping deletion of ref %s", ref.getName());
          continue;
        }
