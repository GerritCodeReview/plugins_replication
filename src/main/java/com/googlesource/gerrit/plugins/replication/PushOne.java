...
class PushOne implements ProjectRunnable, CanceledWhileRunning {
  ...
  void addRef(String ref) {
    if (ALL_REFS.equals(ref)) {
      ...
      pushAllRefs = true;
      repLog.atFinest().log(
          "[%s] Added all refs for replication to %s", HexFormat.fromInt(id), uri);
    } else if (!pushAllRefs) {
      delta.add(ref);
      repLog.atFinest().log(
          "[%s] Added ref %s for replication to %s", HexFormat.fromInt(id), ref, uri);
    }
  }
  ...
  private void runPushOperation() {
    ...
    if (!status.isAllowed()) {
      if (status.isCanceled()) {
        repLog.atInfo().log(
            "[%s] PushOp for replication to %s was canceled and thus won't be rescheduled",
            HexFormat.fromInt(id), uri);
      } else {
        repLog.atInfo().log(
    ...
    try {
      ...
      if (msg.contains("access denied")
        ...
        repLog.atSevere().log(
            "[%s] Cannot replicate %s; Remote repository error: %s",
            HexFormat.fromInt(id), projectName, msg);
      }
      ...
      if (cause instanceof JSchException && cause.getMessage().startsWith("UnknownHostKey:")) {
        repLog.atSevere().log(
            "[%s] Cannot replicate to %s: %s", HexFormat.fromInt(id), uri, cause.getMessage());
      } else if (e instanceof LockFailureException) {
        lockRetryCount++;
        repLog.atSevere().log(
            "[%s] Cannot replicate to %s due to lock failure", HexFormat.fromInt(id), uri);
        ...
        if (lockRetryCount <= maxLockRetries) {
          ...
          repLog.atSevere().log(
              "[%s] Giving up after %d lock failures during replication to %s",
              HexFormat.fromInt(id), lockRetryCount, uri);
        }
      } else {
        if (canceledWhileRunning.get()) {
          logCanceledWhileRunningException(e);
          ...
          repLog.atSevere().withCause(e).log(
              "[%s] Cannot replicate to %s", HexFormat.fromInt(id), uri);
          // The remote push operation should be retried.
          pool.reschedule(this, Destination.RetryReason.TRANSPORT_ERROR);
        }
      }
  ...
  private void logCanceledWhileRunningException(TransportException e) {
    repLog.atInfo().withCause(e).log(
        "[%s] Cannot replicate to %s. It was canceled while running", HexFormat.fromInt(id), uri);
  }
      ...
      try {
        ...
        if (replicationQueue.createProject(
          ...
          repLog.atWarning().log(
              "[%s] Missing repository created; retry replication to %s",
              HexFormat.fromInt(id), uri);
          pool.reschedule(this, Destination.RetryReason.REPOSITORY_MISSING);
        } else {
          repLog.atWarning().log(
              "[%s] Missing repository could not be created when replicating %s",
              HexFormat.fromInt(id), uri);
        }
