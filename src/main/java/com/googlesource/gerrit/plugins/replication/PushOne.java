...
class PushOne implements ProjectRunnable {
  ...
  static final int MAX_LOCK_TRIES = 3;
  ...
  private void runPushOperation() {
    ...
    if (!canceled) {
      try {
        ...
        if (lockRetry < MAX_LOCK_TRIES) {
          pool.reschedule(this);
        } else {
          log.error("Giving up after " + MAX_LOCK_TRIES +
              " attempts to replicate, but still failing due to: " + e.getMessage());
        }
      } catch (IOException e) {
        log.error("Cannot replicate to " + uri, e);
