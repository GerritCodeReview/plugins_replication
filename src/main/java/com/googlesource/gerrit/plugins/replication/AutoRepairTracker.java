...
public class AutoRepairTracker {
  ...
  public boolean tryBeginRepair(Project.NameKey project, URIish uri) {
    ...
    statesByDestination.compute(
        key,
        (k, current) -> {
          if (!canBeginRepair(current, now, intervalMs, maxAttempts, project, uri)) {
            return current;
          }
          allowed[0] = true;
          return new State(current == null ? 1 : current.repairAttemptCount + 1, now);
        });
    return allowed[0];
  }

  private static boolean canBeginRepair(
      State current,
      long now,
      long intervalMs,
      int maxAttempts,
    ...
    if (current.repairAttemptCount >= maxAttempts) {
      repLog.atWarning().log(
          "Skipping auto-repair for %s to %s: reached max attempts (%d/%d)",
          project.get(), uri, current.repairAttemptCount, maxAttempts);
      return false;
    }
