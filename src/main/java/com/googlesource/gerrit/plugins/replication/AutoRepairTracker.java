...
public class AutoRepairTracker {
  ...
  public boolean tryBeginRepair(Project.NameKey project, URIish uri) {
    ...
    DestinationKey key = new DestinationKey(project, uri);
    long now = System.currentTimeMillis();
    State state = statesByDestination.computeIfAbsent(key, k -> new State());
    return state.tryBeginRepair(now, intervalMs, maxAttempts, project, uri);
  }

  private record DestinationKey(Project.NameKey project, URIish uri) {}

  private static class State {
    private int repairAttemptCount;
    private long repairStartTimestamp;

    synchronized boolean tryBeginRepair(
        long now, long intervalMs, int maxAttempts, Project.NameKey project, URIish uri) {
      if (!canBeginRepair(now, intervalMs, maxAttempts, project, uri)) {
        return false;
      }
      repairAttemptCount++;
      repairStartTimestamp = now;
      return true;
    }

    private boolean canBeginRepair(
        long now, long intervalMs, int maxAttempts, Project.NameKey project, URIish uri) {
      if (repairAttemptCount >= maxAttempts) {
        repLog.atWarning().log(
            "Skipping auto-repair for %s to %s: reached max attempts (%d/%d)",
            project.get(), uri, repairAttemptCount, maxAttempts);
        return false;
      }
      long remainingMs = intervalMs - (now - repairStartTimestamp);
      if (intervalMs > 0 && remainingMs > 0) {
        ...
        repLog.atInfo().log(
            "Skipping auto-repair for %s to %s: interval not elapsed."
                + " Next attempt allowed in %.1f hours",
