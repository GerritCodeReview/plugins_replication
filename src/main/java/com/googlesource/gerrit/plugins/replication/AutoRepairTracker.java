...
public class AutoRepairTracker {
  ...
  public boolean tryBeginRepair(Project.NameKey project, URIish uri) {
    ...
    State current = statesByDestination.get(key);
    if (current != null && !current.canBeginRepair(now, intervalMs, maxAttempts, project, uri)) {
      return false;
    }
    statesByDestination.compute(key, (k, c) -> c == null ? new State(1, now) : c.nextAttempt(now));
    return true;
  }
  ...
  private record State(int repairAttemptCount, long repairStartTimestamp) {
    ...
    State nextAttempt(long now) {
      return new State(repairAttemptCount + 1, now);
    }
  }
}
