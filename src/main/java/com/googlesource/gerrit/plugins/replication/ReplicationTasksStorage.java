...
public class ReplicationTasksStorage {
  ...
  public synchronized void resetAll() {
    for (ReplicateRefUpdate r : running()) {
      new Task(r).reset();
    }
  }
  ...
  public synchronized Set<ReplicateRefUpdate> waiting() {
    return replicateRefUpdates(createDir(waitingUpdates));
  }

  @VisibleForTesting
  public synchronized Set<ReplicateRefUpdate> running() {
    return replicateRefUpdates(createDir(runningUpdates));
  }

  @VisibleForTesting
  public synchronized Set<ReplicateRefUpdate> building() {
    return replicateRefUpdates(createDir(buildingUpdates));
  }

  @VisibleForTesting
  public synchronized Set<ReplicateRefUpdate> replicateRefUpdates() {
