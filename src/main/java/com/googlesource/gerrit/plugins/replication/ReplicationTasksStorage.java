...
public class ReplicationTasksStorage {
  ...
  public List<ReplicateRefUpdate> listWaiting() {
    return listRefs(createDir(waitingUpdates));
  }

  public List<ReplicateRefUpdate> listRunning() {
    return listRefs(createDir(runningUpdates));
  }
  ...
  private List<ReplicateRefUpdate> listRefs(Path dir) {
    ...
    public void finish() {
      ...
      try {
        ...
        try {
          ...
          logger.atFine().log("Error cleaning up the task %s", taskKey);
        }
      } catch (IOException e) {
