...
public class ReplicationTasksStorage {
  ...
  public void abortRunning(ReplicateRefUpdate r) {
    new Task(r).abortRunning();
  }

  public void abortAllRunning() {
    for (ReplicateRefUpdate r : list(runningUpdates())) {
      abortRunning(r);
    }
  }
  ...
  public List<ReplicateRefUpdate> list() {
    List<ReplicateRefUpdate> results = list(waitingUpdates());
    results.addAll(list(runningUpdates()));
    return results;
  }
  ...
  private List<ReplicateRefUpdate> list(Path tasks) {
    ...
    try (DirectoryStream<Path> events = Files.newDirectoryStream(tasks)) {
      ...
      logger.atSevere().withCause(e).log("Error when listing pending tasks");
    }
    return results;
  }
