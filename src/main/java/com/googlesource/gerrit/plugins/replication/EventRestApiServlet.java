...
public class EventRestApiServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse res) {
    Optional<StartReplicationRequest> startReplicationRequestOpt;

    try {
      startReplicationRequestOpt = createStartReplicationRequest(req);
    } catch (IOException e) {
    ...
    if (!startReplicationRequestOpt.isPresent()) {
      sendErrorMessageResponse(res, HttpServletResponse.SC_BAD_REQUEST);
      return;
    }

    startReplicationRequest = startReplicationRequestOpt.get();
    repLog.debug(
        "Received command={}, project={}, url={}, now={}, wait={}",
        startReplicationRequest.getCommand(),
        startReplicationRequest.getProject(),
        startReplicationRequest.getUrl(),
        startReplicationRequest.isNow(),
        startReplicationRequest.isWait());

    try {
      startReplicationProcessing.execute(startReplicationRequest, res);
    } catch (IOException e) {
  ...
  private Optional<StartReplicationRequest> createStartReplicationRequest(
    ...
    try {
      CommandType command = startReplicationRequest.getCommand();
      if (command == CommandType.ALL
          || ((command == CommandType.PROJECT) && (startReplicationRequest.getProject() != null))) {
