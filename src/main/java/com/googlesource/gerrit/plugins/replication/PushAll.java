...
public class PushAll implements Runnable {
  ...
  public interface Factory {
    PushAll create(
        ...
        @Assisted("refSpec") String refSpec,
    ...
    try {
      for (Project.NameKey nameKey : projectCache.all()) {
        if (filter.matches(nameKey)) {
          replication.scheduleFullSync(nameKey, urlMatch, refSpec, remotesToConsider, state, now);
        }
      }
    } catch (Exception e) {
  ...
  public String toString() {
    String refs = PushOne.ALL_REFS.equals(refSpec) ? "All Refs" : "[" + refSpec + "]";
