...
_NOTE:_ make sure the local user's ssh keys format is PEM, here how to generate them:
...
are two refs (change ref and meta ref) e.g.,
...
If two or more replication operations (to the same git repository and ref)
...
[1]: http://git-scm.com/docs/git-push#URLS
...
by hostname of another Gerrit server, e.g.,
...
[delete-project](https://gerrit.googlesource.com/plugins/delete-project) plugin
must be installed on the other Gerrit.

_Backward compatibility notice_

Before Gerrit v2.13 it was possible to enable replication to different
Gerrit masters using `gerrit+ssh://`
as protocol name followed by hostname of another Gerrit server, e.g.,
...
system, if using the SSH transport. Defaults to `git-receive-pack`.

`remote.NAME.uploadpack`
: Path of the `git-upload-pack` executable on the remote system,
if using the SSH transport. Defaults to `git-upload-pack`.
...
_NOTE:_ If an `authGroup` is provided, and you want a complete
...
By default, false, do _not_ replicate project deletions.

`remote.NAME.replicateHiddenProjects`
: If true, hidden projects will be replicated to the remote site.

By default, false, do _not_ replicate hidden projects.
...
See [Metrics](metrics.md) for further details. Default: 15 minutes
