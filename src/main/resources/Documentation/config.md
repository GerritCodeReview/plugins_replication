...
:	Number of times to retry a replication operation if a lock
	...
	are scheduled at approximately same time (and end up on different
	...
	you see lockError collisions in you server logs. A too highly set
	value risks keeping around the replication operations in the queue
	for a long time, and the number of items in queue will increase with
	time.
