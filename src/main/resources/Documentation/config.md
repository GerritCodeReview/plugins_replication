...
:	Address of the alternative remote server only for repository
	...
	as protocol name followed by hostname of slave Gerrit server eg.

	`gerrit://replica1.my.org/`

	In this case replication will use Gerrit's SSH API to
	create/remove projects and update repository HEAD refereces.

	NOTE: In order to replicate project deletion, the delete-project
        plugin must be installed on the slave Gerrit.
