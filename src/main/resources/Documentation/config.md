...
the replication event persistence subsystem. Thus, the directory
...
other nodes will pick up some of the work it triggers. Then it no longer
...
enabled. Scaling down such thread requirements per remote results in a
reduced memory footprint per remote on each node in the cluster. This
